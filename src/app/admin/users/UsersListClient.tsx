...
import { Users, Search, Loader2, MoreVertical, ShieldAlert, CheckCircle2 } from "lucide-react";

const fmt = (n: number) => (Number(n) || 0).toLocaleString("fr-FR");

export default function UsersListClient({ initialUsers = [] }: { initialUsers?: any[] }) {
  const { users, isLoading, error, mutate } = useAdminUsers(initialUsers);
          ...
          <table className="w-full text-left text-sm text-neutral-600">
            <thead className="bg-neutral-50 border-b border-neutral-200 text-xs font-bold text-neutral-500 uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4 text-center">Dépenses (Total)</th>
