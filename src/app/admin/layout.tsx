...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-neutral-900 text-neutral-100 antialiased selection:bg-orange-500 selection:text-white">
      ...
      {/* Sidebar Navigation */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-50 flex flex-col bg-neutral-950 border-r border-neutral-800 transition-all duration-300 ease-in-out lg:sticky ${
          mobileMenuOpen ? "translate-x-0 w-72" : "-translate-x-full lg:translate-x-0"
        } ${collapsed ? "lg:w-20" : "lg:w-64"}`}
      >
        {/* Sidebar Header */}
        <div className="flex items-center justify-between h-16 px-4 border-b border-neutral-800/80">
          <Link href="/admin" className="flex items-center gap-3 overflow-hidden group">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-orange-600 via-amber-500 to-orange-400 flex items-center justify-center text-white shadow-lg shadow-orange-500/20 shrink-0 group-hover:scale-105 transition-transform">
              <Sparkles className="w-5 h-5" />
            </div>
            {(!collapsed || mobileMenuOpen) && (
              <div className="flex flex-col min-w-0">
                <span className="font-heading text-lg font-black tracking-tight text-white flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-neutral-400 truncate uppercase tracking-widest font-semibold">
                  Control Center
                </span>
              </div>
            )}
          ...
          <nav className="space-y-1" aria-label="Navigation Administrateur">
            {adminNavItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={`w-5 h-5 shrink-0 ${active ? "text-white" : "text-neutral-400 group-hover:text-white"}`} />
                  {(!collapsed || mobileMenuOpen) && (
        ...
        <div className="p-3 border-t border-neutral-800/80 bg-neutral-950/80 space-y-2">
          <Link
            href="/dashboard"
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-neutral-400 hover:text-white hover:bg-neutral-800/70 transition-colors text-sm font-medium"
            title={collapsed ? "Retour à l'App" : undefined}
          >
            <ArrowLeft className="w-5 h-5 shrink-0 text-neutral-400" />
          ...
          <button
            ...
            className="hidden lg:flex items-center justify-center w-full py-2 rounded-xl text-neutral-500 hover:text-neutral-200 hover:bg-neutral-800/50 transition-colors text-xs font-medium gap-2"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top Navigation Bar */}
        <header className="h-16 sticky top-0 z-30 bg-neutral-950/90 backdrop-blur-md border-b border-neutral-800/80 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {/* Mobile menu trigger */}
            <button
              onClick={() => setMobileMenuOpen(true)}
              className="p-2 rounded-lg text-neutral-400 hover:text-white hover:bg-neutral-800 lg:hidden"
            ...
            <div className="flex items-center gap-2.5">
              <span className="font-heading font-bold text-white text-base sm:text-lg hidden sm:inline">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <Link
              href="/dashboard"
              className="hidden md:inline-flex items-center gap-1.5 text-xs font-semibold text-neutral-400 hover:text-white px-3 py-1.5 rounded-lg border border-neutral-700/60 hover:border-neutral-600 transition-colors"
            >
            ...
            <div className="flex items-center gap-3 pl-1">
              ...
              <div className="hidden sm:flex flex-col text-left">
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-bold text-white leading-none">{adminName}</span>
                ...
                <span className="text-[11px] text-neutral-400 leading-tight truncate max-w-[150px]">{adminEmail}</span>
              </div>
            </div>
          </div>
        </header>
