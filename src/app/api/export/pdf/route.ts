...
/**
 * Load pdfmake's Node printer and its bundled Roboto fonts as Buffers.
 * Using Buffers (not filesystem paths) keeps this working inside the
 * serverless bundle without shipping separate .ttf files.
 */
...
function buildDocDefinition(
  ...
  // --- Title page ---
  content.push({
    text: (title || "Sans titre").toUpperCase(),
    style: "coverTitle",
    margin: [0, 220, 0, 10],
  });
  if (subtitle) {
    content.push({ text: subtitle, style: "coverSubtitle" });
  }
  content.push({
    text: "───────────────",
    alignment: "center",
...
export async function POST(req: Request) {
  try {
    ...
    const docDefinition = buildDocDefinition(title || "Mon Livre Iris", subtitle, chapters);
