  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] font-body text-neutral-900 dark:text-neutral-100 flex flex-col md:flex-row">
      ...
      <div className="flex-1 flex flex-col min-w-0 pb-20 md:pb-10">
        <header className="bg-white dark:bg-neutral-900/80 backdrop-blur-md sticky top-0 z-30 h-16 px-4 md:px-8 flex items-center justify-between gap-4 border-b border-neutral-100 dark:border-neutral-800">
          <div className="flex items-center gap-3">
            <Link href="/analytics" className="hidden sm:flex items-center gap-1.5 text-xs font-bold text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:text-neutral-100 bg-neutral-100 dark:bg-neutral-800 px-3 py-2 rounded-xl transition-all">
        ...
        <main className="p-4 sm:p-6 md:p-10 max-w-5xl mx-auto w-full space-y-8">
          ...
          {loading ? (
            <div className="flex flex-col justify-center items-center h-64 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl">
              <span className="material-symbols-outlined animate-spin text-secondary text-4xl">progress_activity</span>
              <p className="text-sm font-semibold text-neutral-500 dark:text-neutral-400 mt-4">Analyse de votre manuscrit en cours...</p>
            </div>
          ) : !project ? (
            <div className="p-12 text-center text-neutral-500 dark:text-neutral-400 bg-white dark:bg-neutral-900 rounded-3xl border border-neutral-200 dark:border-neutral-800">
              <p className="font-bold text-neutral-800 dark:text-neutral-200">Projet introuvable</p>
            </div>
          ) : (
            <>
              {/* Project Info Header */}
              <div className="bg-white dark:bg-neutral-900 p-8 rounded-3xl border border-neutral-200/80 dark:border-neutral-800 shadow-2xs flex flex-col sm:flex-row items-center sm:items-start gap-6">
                {project.cover_url ? (
                  <div className="w-24 h-36 rounded-lg shadow-sm overflow-hidden shrink-0 bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-800">
                    <img src={project.cover_url} alt="Cover" className="w-full h-full object-cover" />
                  </div>
                ) : (
                  <div className="w-24 h-36 rounded-lg shadow-sm bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-800 shrink-0 flex items-center justify-center">
                ...
                <div className="flex-1 text-center sm:text-left space-y-2">
                  ...
                  <h2 className="text-3xl font-heading font-extrabold text-neutral-900 dark:text-neutral-100">{project.title}</h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white dark:bg-neutral-900 p-6 rounded-3xl border border-neutral-200/80 dark:border-neutral-800 shadow-2xs text-center space-y-2">
                  <FileText className="w-6 h-6 text-blue-500 mx-auto mb-3" />
                  <span className="text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider block">Mots</span>
                  <p className="font-heading font-extrabold text-2xl text-neutral-900 dark:text-neutral-100">{totalWords.toLocaleString('fr-FR')}</p>
                </div>
                
                <div className="bg-white dark:bg-neutral-900 p-6 rounded-3xl border border-neutral-200/80 dark:border-neutral-800 shadow-2xs text-center space-y-2">
                  <BookOpen className="w-6 h-6 text-emerald-500 mx-auto mb-3" />
                  <span className="text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider block">Pages (KDP)</span>
                  <p className="font-heading font-extrabold text-2xl text-neutral-900 dark:text-neutral-100">{totalPages}</p>
                </div>
                
                <div className="bg-white dark:bg-neutral-900 p-6 rounded-3xl border border-neutral-200/80 dark:border-neutral-800 shadow-2xs text-center space-y-2">
                  <Clock className="w-6 h-6 text-purple-500 mx-auto mb-3" />
                  <span className="text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider block">Temps de lecture</span>
                  <p className="font-heading font-extrabold text-2xl text-neutral-900 dark:text-neutral-100">{readingTime}</p>
                </div>

                <div className="bg-white dark:bg-neutral-900 p-6 rounded-3xl border border-neutral-200/80 dark:border-neutral-800 shadow-2xs text-center space-y-2">
                  <Coins className="w-6 h-6 text-amber-500 mx-auto mb-3" />
                  <span className="text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider block">Coût estimé</span>
                  <p className="font-heading font-extrabold text-2xl text-neutral-900 dark:text-neutral-100">{cost}</p>
                </div>
              </div>

              {/* Word Count Distribution Chart */}
              <div className="bg-white dark:bg-neutral-900 p-6 md:p-8 rounded-3xl border border-neutral-200/80 dark:border-neutral-800 shadow-2xs space-y-6">
                ...
                {chapterStats.length === 0 ? (
                  <p className="text-sm text-neutral-500 dark:text-neutral-400 italic">Aucun chapitre créé pour l'instant.</p>
                ) : (
                      ...
                      return (
                        <div key={idx} className="space-y-1.5">
                          <div className="flex justify-between items-center text-xs">
                            <span className="font-bold text-neutral-800 dark:text-neutral-200 line-clamp-1">{stat.title || `Chapitre ${idx + 1}`}</span>
                            <span className="text-neutral-500 dark:text-neutral-400 font-medium">{stat.words.toLocaleString('fr-FR')} mots</span>
                          </div>
