    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-neutral-900/50 backdrop-blur-xs p-0 sm:p-4">
      <div className="bg-white dark:bg-neutral-900 w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl border border-neutral-100 dark:border-neutral-800 max-h-[92dvh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-neutral-100 dark:border-neutral-800 sticky top-0 bg-white dark:bg-neutral-900 z-10">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center text-neutral-400 hover:bg-neutral-100 dark:bg-neutral-800 hover:text-neutral-700 dark:text-neutral-300 transition-colors">
        ...
        <div className="px-5 sm:px-6 py-5 space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-neutral-800 dark:text-neutral-200">Longueur du livre (approximative)</label>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className={`text-sm font-extrabold ${active ? "text-secondary" : "text-neutral-800 dark:text-neutral-200"}`}>{p.label}</span>
                    ...
                    <p className="text-[11px] font-bold text-neutral-500 dark:text-neutral-400 mt-0.5">{p.pages}</p>
          ...
          <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 overflow-hidden">
            <div className="px-4 py-2.5 bg-neutral-50 dark:bg-neutral-800/50 border-b border-neutral-100 dark:border-neutral-800 flex items-center justify-between">
              <span className="text-xs font-bold text-neutral-700 dark:text-neutral-300">Coût estimé pour la rédaction totale</span>
                ...
                return (
                  <div key={m.id} className={`flex items-center justify-between px-4 py-2.5 ${active ? "bg-orange-50/50" : ""}`}>
                    <span className="flex items-center gap-2 text-xs">
                      ...
                      <span className={`font-bold ${active ? "text-secondary" : "text-neutral-700 dark:text-neutral-300"}`}>{m.label}</span>
                      <span className="text-neutral-400">{m.hint.replace(/^.*—\s*/, "")}</span>
                    </span>
                    <span className={`text-sm font-bold ${active ? "text-neutral-900 dark:text-neutral-100" : "text-neutral-600 dark:text-neutral-400"}`}>{fmt(m.coins)} crédits</span>
                  </div>
                );
              })}
            </div>
